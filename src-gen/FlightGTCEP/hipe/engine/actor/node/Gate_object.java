...
public class Gate_object extends AbstractActor {
	
	private List<Port<Flights.Gate>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<Flights.Gate>(getSelf(), name2actor.get("Flight_src_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<Flights.Gate>(getSelf(), name2actor.get("Flight_trg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<Flights.Gate> message) {
		for(Port<Flights.Gate> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<Flights.Gate> message) {
		for(Port<Flights.Gate> port : ports) {
	...
	private void changeAttribute(AttributeChanged<Flights.Gate> message) {
		for(Port<?> port : ports) {
