...
public class Travel_object extends AbstractActor {
	
	private List<Port<Flights.Travel>> ports;		
		
	public Travel_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<Flights.Travel>(getSelf(), name2actor.get("Flight_travels_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<Flights.Travel>(getSelf(), name2actor.get("Travel_flights_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<Flights.Travel> message) {
		for(Port<Flights.Travel> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<Flights.Travel> message) {
		for(Port<Flights.Travel> port : ports) {
	...
	private void changeAttribute(AttributeChanged<Flights.Travel> message) {
		for(Port<?> port : ports) {
