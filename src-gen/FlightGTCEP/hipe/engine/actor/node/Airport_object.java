package FlightGTCEP.hipe.engine.actor.node;
...
public class Airport_object extends AbstractActor {
	
	private List<Port<Flights.Airport>> ports;		
		
	public Airport_object() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeRight<Flights.Airport>(getSelf(), name2actor.get("Route_src_0_reference"), this::returnTrue));
		ports.add(new PortNodeRight<Flights.Airport>(getSelf(), name2actor.get("Route_trg_0_reference"), this::returnTrue));
		ports.add(new PortNodeMatch<Flights.Airport>(getSelf(), name2actor.get("examplePattern_production"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<Flights.Airport> message) {
		for(Port<Flights.Airport> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<Flights.Airport> message) {
		for(Port<Flights.Airport> port : ports) {
	...
	private void changeAttribute(AttributeChanged<Flights.Airport> message) {
		for(Port<?> port : ports) {
