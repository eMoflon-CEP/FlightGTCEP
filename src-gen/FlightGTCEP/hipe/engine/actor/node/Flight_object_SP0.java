...
public class Flight_object_SP0 extends AbstractActor {
	
	private List<Port<Flights.Flight>> ports;		
		
	public Flight_object_SP0() {
	}

	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortNodeLeft<Flights.Flight>(getSelf(), name2actor.get("Flight_src_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<Flights.Flight>(getSelf(), name2actor.get("Flight_travels_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<Flights.Flight>(getSelf(), name2actor.get("Flight_trg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<Flights.Flight> message) {
		for(Port<Flights.Flight> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<Flights.Flight> message) {
		for(Port<Flights.Flight> port : ports) {
	...
	private void changeAttribute(AttributeChanged<Flights.Flight> message) {
		for(Port<?> port : ports) {
