...
public class Route_object extends AbstractActor {
	
	private List<Port<Flights.Route>> ports;		
	...
	public void initActor(InitActor m) {
		...
		ports.add(new PortNodeRight<Flights.Route>(getSelf(), name2actor.get("Flight_route_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<Flights.Route>(getSelf(), name2actor.get("Route_src_0_reference"), this::returnTrue));
		ports.add(new PortNodeLeft<Flights.Route>(getSelf(), name2actor.get("Route_trg_0_reference"), this::returnTrue));
	}
	...
	private void addNode(ObjectAdded<Flights.Route> message) {
		for(Port<Flights.Route> port : ports) {
			port.sendAdd(message, message.node);
		}
		message.initialMessage.decrement();
	}
	
	private void removeNode(ObjectDeleted<Flights.Route> message) {
		for(Port<Flights.Route> port : ports) {
	...
	private void changeAttribute(AttributeChanged<Flights.Route> message) {
		for(Port<?> port : ports) {
