...
public class Route_trg_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<Flights.Route, Set<EdgeMatch>> sourceElements = new HashMap<Flights.Route, Set<EdgeMatch>>();
	private Map<Flights.Airport, Set<EdgeMatch>> targetElements = new HashMap<Flights.Airport, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<Flights.Airport, Set<Flights.Route>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Route_trg_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("replacmentConnectingFlight_10_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_16_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_24_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("travelHasConnectingFlight_47_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, Flights.Route source, Flights.Airport target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<Flights.Route> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<Flights.Route>();
			pending.put((Flights.Airport) target, sourcePending);
		}
		sourcePending.add((Flights.Route) source);
	}
	
	private void leftAdded(NodeAddedLeft<Flights.Route> message) {
		...
		Flights.Airport target = (Flights.Airport) message.input.getTrg();
	...
	private void rightAdded(NodeAddedRight<Flights.Airport> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<Flights.Route> pendingsSources = pending.get(message.input);
			for(Flights.Route source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<Flights.Route> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<Flights.Route> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<Flights.Airport> message) {
	...
	private void addReference(ReferenceAdded<Flights.Route, Flights.Airport> message) {
	...
	private void removeReference(ReferenceDeleted<Flights.Route, Flights.Airport> message) {
		Set<Flights.Route> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof Flights.Route) {
			leftChanged(message);
		}
		else {
