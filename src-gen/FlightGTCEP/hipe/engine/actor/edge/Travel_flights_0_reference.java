...
public class Travel_flights_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<Flights.Travel, Set<EdgeMatch>> sourceElements = new HashMap<Flights.Travel, Set<EdgeMatch>>();
	private Map<Flights.Flight, Set<EdgeMatch>> targetElements = new HashMap<Flights.Flight, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<Flights.Flight, Set<Flights.Travel>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Travel_flights_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_33_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("replacmentConnectingFlight_33_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("travelHasConnectingFlight_61_junction"), this::returnTrue));
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("travelHasConnectingFlight_61_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, Flights.Travel source, Flights.Flight target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<Flights.Travel> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<Flights.Travel>();
			pending.put((Flights.Flight) target, sourcePending);
		}
		sourcePending.add((Flights.Travel) source);
	}
	
	private void leftAdded(NodeAddedLeft<Flights.Travel> message) {
		...
		for(Flights.Flight target : message.input.getFlights()) {
	...
	private void rightAdded(NodeAddedRight<Flights.Flight> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<Flights.Travel> pendingsSources = pending.get(message.input);
			for(Flights.Travel source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<Flights.Travel> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<Flights.Travel> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<Flights.Flight> message) {
	...
	private void addReference(ReferenceAdded<Flights.Travel, Flights.Flight> message) {
	...
	private void removeReference(ReferenceDeleted<Flights.Travel, Flights.Flight> message) {
		Set<Flights.Travel> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof Flights.Travel) {
			leftChanged(message);
		}
		else {
