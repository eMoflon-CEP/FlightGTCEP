package FlightGTCEP.hipe.engine.actor.edge;
...
public class Flight_route_0_reference extends AbstractActor {

	private List<Port<EdgeMatch>> ports;

	private Map<Flights.Flight, Set<EdgeMatch>> sourceElements = new HashMap<Flights.Flight, Set<EdgeMatch>>();
	private Map<Flights.Route, Set<EdgeMatch>> targetElements = new HashMap<Flights.Route, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<Flights.Route, Set<Flights.Flight>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Flight_route_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("flightWithRoute_production"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_19_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_27_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("replacmentConnectingFlight_38_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("travelHasConnectingFlight_47_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("travelHasConnectingFlight_53_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, Flights.Flight source, Flights.Route target) {
	...
	private void addPendingMatch(Object source, Object target) {
		Set<Flights.Flight> sourcePending = pending.get(target);
		if(sourcePending == null) {
			sourcePending = new HashSet<Flights.Flight>();
			pending.put((Flights.Route) target, sourcePending);
		}
		sourcePending.add((Flights.Flight) source);
	}
	
	private void leftAdded(NodeAddedLeft<Flights.Flight> message) {
		...
		Flights.Route target = (Flights.Route) message.input.getRoute();
	...
	private void rightAdded(NodeAddedRight<Flights.Route> message) {
		...
		if(pending.containsKey(message.input)) {
			Set<Flights.Flight> pendingsSources = pending.get(message.input);
			for(Flights.Flight source : pendingsSources) {
	...
	private void leftRemoved(NodeDeletedLeft<Flights.Flight> message) {
		...
		for(EdgeMatch match : sourceMatches) {
			...
			Set<Flights.Flight> sourcePending = pending.get(match.target());
	...
	private void rightRemoved(NodeDeletedRight<Flights.Route> message) {
	...
	private void addReference(ReferenceAdded<Flights.Flight, Flights.Route> message) {
	...
	private void removeReference(ReferenceDeleted<Flights.Flight, Flights.Route> message) {
		Set<Flights.Flight> pendingSources = pending.get(message.target);
	...
	private void changeAttribute(AttributeChanged<?> message) {
		...
		if(message.node instanceof Flights.Flight) {
			leftChanged(message);
		}
		else {
