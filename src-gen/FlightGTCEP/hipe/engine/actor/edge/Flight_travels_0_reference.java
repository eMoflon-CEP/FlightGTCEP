...
public class Flight_travels_0_reference extends AbstractActor {
	...
	private Map<Flights.Travel, Set<EdgeMatch>> targetElements = new HashMap<Flights.Travel, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<Flights.Travel, Set<Flights.Flight>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Flight_travels_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdge(getSelf(), name2actor.get("flightInTravel_production"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, Flights.Flight source, Flights.Travel target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((Flights.Travel) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<Flights.Flight> message) {
		...
		for(Flights.Travel target : message.input.getTravels()) {
	...
	private void rightAdded(NodeAddedRight<Flights.Travel> message) {
	...
	private void rightRemoved(NodeDeletedRight<Flights.Travel> message) {
	...
	private void addReference(ReferenceAdded<Flights.Flight, Flights.Travel> message) {
	...
	private void removeReference(ReferenceDeleted<Flights.Flight, Flights.Travel> message) {
