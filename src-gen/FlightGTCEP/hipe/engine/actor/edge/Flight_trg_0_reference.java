...
public class Flight_trg_0_reference extends AbstractActor {
	...
	private Map<Flights.Gate, Set<EdgeMatch>> targetElements = new HashMap<Flights.Gate, Set<EdgeMatch>>();
	private HiPESet<EdgeMatch> matches = new HiPESet<>();
	
	private Map<Flights.Gate, Set<Flights.Flight>> pending = new HashMap<>();

	private int finishedNodes = 0;
	
	public Flight_trg_0_reference() {
	}
	
	public void initActor(InitActor m) {
		Map<String, ActorRef> name2actor = m.name2actor;
		ports = new LinkedList<>();
		ports.add(new PortEdgeRight(getSelf(), name2actor.get("replacmentConnectingFlight_27_junction"), this::returnTrue));
		ports.add(new PortEdgeLeft(getSelf(), name2actor.get("travelHasConnectingFlight_43_junction"), this::returnTrue));
	}	
	...
	private void addMatch(InputMessage message, Flights.Flight source, Flights.Gate target) {
	...
	private void addPendingMatch(Object source, Object target) {
		...
		if(sourcePending == null) {
			...
			pending.put((Flights.Gate) target, sourcePending);
		}
	...
	private void leftAdded(NodeAddedLeft<Flights.Flight> message) {
		...
		Flights.Gate target = (Flights.Gate) message.input.getTrg();
	...
	private void rightAdded(NodeAddedRight<Flights.Gate> message) {
	...
	private void rightRemoved(NodeDeletedRight<Flights.Gate> message) {
	...
	private void addReference(ReferenceAdded<Flights.Flight, Flights.Gate> message) {
	...
	private void removeReference(ReferenceDeleted<Flights.Flight, Flights.Gate> message) {
